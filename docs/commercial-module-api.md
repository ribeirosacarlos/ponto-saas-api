...
- Roles novas: `commercial_manager`, `commercial_agent` (além de `super_admin`, que sempre tem acesso total).
...
Importante: `commercial_agent` recebe **404** (não 403) ao tentar acessar por ID um lead que não é dele — a query já vem filtrada por `assigned_to_user_id`, então o recurso "não existe" para ele. Isso evita confirmar a existência de leads de outros agentes.
...
`{ "name", "slug" (único, alpha_dash), "email?", "phone?", "commission_plan_id?", "status?" }`
